...
tribute("nontype_functional", "BSD-2-Clause",
        "https://github.com/zhihaoy/nontype_functional")
tribute("concurrentqueue", "BSD-2-Clause",
        "https://github.com/cameron314/concurrentqueue")
tribute("stack_alloc", "MIT",
        "https://raw.githubusercontent.com/charles-salvia/charles/master/stack_allocator.hpp")
...
function nielib_data()
    ...
    add_defines("GLM_ENABLE_EXPERIMENTAL", "GLM_FORCE_DEPTH_ZERO_TO_ONE",
    ...
    add_defines("GLM_FORCE_RADIANS", "GLM_ENABLE_EXPERIMENTAL",
                "GLM_FORCE_DEPTH_ZERO_TO_ONE", {public = true, force = true})
    ...
    add_cxxflags("-Werror=inconsistent-missing-override",
                 {public = true, force = true})
    add_cxflags("-fuse-ld=lld", "-Werror=move", "-Werror=unused-result",
                "-Werror=return-type", "-Werror=switch",
                "-Werror=delete-non-virtual-dtor", "-Werror=return-type",
                "-Werror=switch", "-Werror=call-to-pure-virtual-from-ctor-dtor",
                "-Werror=defaulted-function-deleted",
                "-Werror=delete-non-virtual-dtor",
                "-Werror=abstract-final-class", "-ftemplate-backtrace-limit=0",
                "-Werror=ignored-attributes", "-Werror=unused-value",
                "-Werror=uninitialized",
                "-Werror=tautological-constant-out-of-range-compare",
                "-Werror=argument-undefined-behaviour", "-Werror=vexing-parse",
                "-Werror=reorder-ctor", "-Werror=dangling",
                {public = true, force = true})
    ...
    if is_os("linux") then
        ...
        add_cxflags("-fPIC", "-fuse-ld=lld", "-fno-strict-aliasing",
                    "-gdwarf-4", "-rdynamic", {public = true, force = true})
        add_shflags("-fPIC", "-fuse-ld=lld", "-fno-strict-aliasing",
                    "-gdwarf-4", "-rdynamic", {public = true, force = true})
        add_ldflags("-fPIC", "-fuse-ld=lld", "-fno-strict-aliasing",
                    "-gdwarf-4", "-rdynamic", {public = true, force = true})
      ...
      add_shflags("-fsanitize=safe-stack", {public = true, force = true})]]
            ...
            add_cxflags("-fno-asynchronous-unwind-tables",
                        {public = true, force = true})
            add_ldflags("-fno-asynchronous-unwind-tables",
                        {public = true, force = true})
            add_shflags("-fno-asynchronous-unwind-tables",
                        {public = true, force = true})
    ...
    before_link("linux", function(target)
        local paths = table.unique(table.join(target:get_from("linkdirs", "*"),
                                              {"/usr/lib/x86_64-linux-gnu/"}))
        target:linker():_tool().nf_link = function(self, lib)
            local has_file = false
            if (lib ~= "dl") and (lib ~= "pthread") and (lib ~= "m") and
                (lib ~= "c") then
                for _, p in ipairs(paths) do
                    has_file = has_file or
                                   os.exists(path.join(p, "lib" .. lib .. ".a"))
                end
            end
            if has_file and not lib:find("^:") then
                ...
                if lib == "util" then
                    print(paths)
                    print(os.exists(path.join("/usr/lib/x86_64-linux-gnu/",
                                              "lib" .. lib .. ".a")))
                    raise "NO!!!!"
                end
                return "-l" .. lib
...
do
    ...
    if is_os("windows") then
        ...
        add_defines("NIE_EXPORT=[[gnu::visibility(\"default\")]]",
                    {public = true})
    end
end
target_end()
