...
namespace nie {
  ...
  template <class R, class... Args> struct _ref_quals_fn_sig<R(Args...) const&> : _cv_fn_sig<R(Args...) const> {
    template <class T> using ref = T&;
  };
  ...
  template <class Sig> struct _full_fn_sig : _ref_quals_fn_sig<Sig>, _noex_traits<false> {};
  ...
  struct _full_fn_sig<R(Args...) const noexcept> : _ref_quals_fn_sig<R(Args...) const>, _noex_traits<true> {};
  ...
  constexpr inline struct {
    ...
    template <class T> constexpr auto operator()(std::reference_wrapper<T> rhs) const noexcept {
      return std::addressof(rhs.get());
    }

  } _take_reference;

  template <class T> constexpr auto _build_reference = [](auto&&... args) { return new T(decltype(args)(args)...); };

  template <class T> constexpr auto _build_reference<T*> = [](auto&&... args) noexcept -> T* { return {decltype(args)(args)...}; };

  template <class T> constexpr auto _build_reference<std::reference_wrapper<T>> = [](auto& rhs) noexcept { return std::addressof(rhs); };
  ...
  struct _move_only_pointer {
    ...
    constexpr _move_only_pointer(_move_only_pointer&& other) noexcept : val(std::exchange(other.val, {})) {}
    ...
    constexpr explicit _move_only_pointer(T* p) noexcept : val{.fp_ = reinterpret_cast<decltype(val.fp_)>(p)} {}
  ...
  template <class T, template <class...> class Primary> inline constexpr bool _is_specialization_of = false;

  template <template <class...> class Primary, class... Args> inline constexpr bool _is_specialization_of<Primary<Args...>, Primary> = true;

  template <class T, template <class...> class Primary>
  inline constexpr bool _does_not_specialize = not _is_specialization_of<std::remove_cvref_t<T>, Primary>;

  template <class S, class = typename _full_fn_sig<S>::function> class move_only_function;

  template <class S, class R, class... Args> class move_only_function<S, R(Args...)> {
    ...
    template <class T> struct inv_quals_f : std::conditional<is_lvalue_only or is_rvalue_only, cvref<T>, cv<T>&> {};
    template <class T> using inv_quals = inv_quals_f<T>::type;

    template <class... T>
    static constexpr bool is_invocable_using =
        std::conditional_t<noex, std::is_nothrow_invocable_r<R, T..., Args...>, std::is_invocable_r<R, T..., Args...>>::value;

    template <class VT> static constexpr bool is_callable_from = is_invocable_using<cvref<VT>> and is_invocable_using<inv_quals<VT>>;

    template <auto f, class VT> static constexpr bool is_callable_as_if_from = is_invocable_using<decltype(f), inv_quals<VT>>;
  ...
  public:
    ...
    inline move_only_function() = default;
    inline move_only_function(nullptr_t) noexcept : move_only_function() {}

    template <class F, class VT = std::decay_t<F>>
    inline move_only_function(F&& f) noexcept(std::is_nothrow_invocable_v<decltype(_take_reference), F>)
    ...
    template <auto f>
    inline move_only_function(nontype_t<f>) noexcept
    ...
    inline move_only_function(nontype_t<f>, T&& x) noexcept(std::is_nothrow_invocable_v<decltype(_take_reference), T>)
    ...
    inline explicit move_only_function(in_place_type_t<T>, Inits&&... inits) noexcept(
    ...
    inline explicit move_only_function(in_place_type_t<T>,
    ...
    inline explicit move_only_function(nontype_t<f>,
        in_place_type_t<T>,
        Inits&&... inits) noexcept( //
        std::is_nothrow_invocable_v<decltype(_build_reference<T>), Inits...>)
    ...
    inline move_only_function(move_only_function&&) = default;
    inline move_only_function& operator=(move_only_function&&) = default;

    inline void swap(move_only_function& other) noexcept {
      std::swap<move_only_function>(*this, other);
    }

    friend inline void swap(move_only_function& lhs, move_only_function& rhs) noexcept {
      lhs.swap(rhs);
    }
    ...
    friend inline bool operator==(move_only_function const& f, nullptr_t) noexcept {
      return !f;
    }
